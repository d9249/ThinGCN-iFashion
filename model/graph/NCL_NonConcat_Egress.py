...
class NCL_NonConcat_Egress(GraphRecommender):
    def __init__(self, conf, training_set, test_set):
        super(NCL_NonConcat_Egress, self).__init__(conf, training_set, test_set)
        args = OptionConf(self.config['NCL_NonConcat_Egress'])
...
class LGCN_Encoder(nn.Module):
    ...
    def _init_model(self):
        def virgo_initialization(tensor):
            stdv = 1. / torch.sqrt(torch.tensor(tensor.size(1), dtype=torch.float))
            return tensor.uniform_(-stdv, stdv)

        embedding_dict = nn.ParameterDict({
            'user_emb': nn.Parameter(virgo_initialization(torch.empty(self.data.user_num, self.latent_size))),
            'item_emb': nn.Parameter(virgo_initialization(torch.empty(self.data.item_num, self.latent_size))),
        })
    ...
    def forward(self):
        ...
        lgcn_all_embeddings = all_embeddings[-1]
